...
def test_release_workflow_builds_windows_installer_and_macos_dmg() -> None:
    ...
    assert "ImageJakdu-0.1.0-windows.exe" in workflow
    assert "ImageJakdu-0.1.0-windows-installer.exe" in workflow
    assert "ImageJakdu-0.1.0-macos.dmg" in workflow
...
def test_windows_installer_script_packages_pyinstaller_exe() -> None:
    ...
    assert 'OutFile "..\\..\\dist\\ImageJakdu-0.1.0-windows-installer.exe"' in installer
